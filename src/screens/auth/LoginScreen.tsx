...
import { View, StyleSheet, Dimensions, KeyboardAvoidingView, Platform, ScrollView, Animated, TouchableOpacity } from 'react-native';
import { Text, TextInput, Button, useTheme, ActivityIndicator, Snackbar, Portal, List, Modal } from 'react-native-paper';
import Icon from 'react-native-vector-icons/MaterialCommunityIcons';
import { useDispatch } from 'react-redux';
import { AppDispatch } from '../../store';
import { loginUser, updateUserName, selectCampus } from '../../store/slices/authSlice';
...
export const LoginScreen = () => {
    ...
    const [selectedCampusLocal, setSelectedCampusLocal] = useState<{ id: string, name: string, code: string } | null>(null);
    ...
    const handleSendOTP = async () => {
        if (!selectedCampusLocal) {
            showError('Please select your campus first');
            return;
        }
        if (!name.trim()) {
    ...
    const handleVerifyOTP = async () => {
        ...
        try {
            ...
            if (result.success && result.userId && result.token && result.refreshToken) {
                // Store campus in state and local storage
                if (selectedCampusLocal) {
                    dispatch(selectCampus({
                        code: selectedCampusLocal.code,
                        name: selectedCampusLocal.name
                    }));
                }
    ...
    const getSnackbarStyle = () => {
        if (errorMsg?.startsWith('✅')) return { backgroundColor: '#2ECC71' };
        if (errorType === ApiErrorType.NETWORK_ERROR) return { backgroundColor: '#E74C3C' };
        if (errorType === ApiErrorType.TIMEOUT_ERROR) return { backgroundColor: '#F39C12' };
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.container}>
            <ScrollView contentContainerStyle={styles.scrollContent}>
                ...
                <Animated.View style={[styles.formContainer, { opacity: fadeAnim, transform: [{ translateY: slideAnim }] }]}>
                    {!isOtpSent ? (
                        <>
                            {/* Campus Dropdown Placeholder */}
                            <TouchableOpacity
                                style={[styles.dropdown, { borderColor: selectedCampusLocal ? theme.colors.primary : '#E0E0E0' }]}
                                onPress={() => setShowCampusModal(true)}
                                disabled={isFetchingCampuses}
                            >
                                <Icon name="office-building" size={20} color={selectedCampusLocal ? theme.colors.primary : '#666'} />
                                <Text style={[styles.dropdownText, !selectedCampusLocal && { color: '#888' }]}>
                                    {selectedCampusLocal ? selectedCampusLocal.name : 'Select Campus'}
                                </Text>
                                {isFetchingCampuses ? (
                            ...
                            <Button
                                ...
                                loading={isLoading}
                                disabled={isLoading || !selectedCampusLocal}
                                style={styles.button}
                    ...
                    <ScrollView style={styles.campusList}>
                        {campuses.map((campus) => (
                            <List.Item
                                ...
                                onPress={() => {
                                    setSelectedCampusLocal(campus);
                                    setShowCampusModal(false);
                                }}
                                style={[
                                    styles.campusItem,
                                    selectedCampusLocal?.id === campus.id && { backgroundColor: theme.colors.primary + '10' }
                                ]}
                            />
                        ))}
                    </ScrollView>
...
const styles = StyleSheet.create({
    ...
    logoPlaceholder: {
        ...
        marginBottom: 24, elevation: 4, shadowColor: '#000',
        shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.2, shadowRadius: 8,
    },
    logoText: { color: '#FFFFFF', fontSize: 32, fontWeight: 'bold' },
    title: { fontWeight: 'bold', color: '#1A1A2E', textAlign: 'center', marginBottom: 8 },
    subtitle: { color: '#666666', textAlign: 'center' },
    ...
    infoBox: {
        ...
        padding: 12, borderRadius: 8, marginBottom: 16, gap: 12,
    },
